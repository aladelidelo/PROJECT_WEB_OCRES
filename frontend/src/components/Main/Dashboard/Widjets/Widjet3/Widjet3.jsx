        ...
        if (e.yearOfRelease !== "N/A" && e.rottenTomatoesScore !== "N/A" && e.type ==="movie") {
          ...
          data = data.map((x) => {
            if (year >= (parseInt(x.name)) && year < (parseInt(x.name) + 10)) {
              x.count++;
              x.total = x.total + rottenTomatoesScore;
            }
            return x;
          })
        }
  ...
  render() {
    return (
      <div className="widjet-container">
        <div className="title">
          <h5> Score des films par décennie</h5>
        </div>
        <div className="graph">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={this.data} margin={{ top: 30, right: 30, bottom: 30, left: 20 }}>
              ...
              <YAxis yAxisId="left" tick={{ fontSize: 12 }}>
                <Label value="Rotten Tomatoes Score" offset={-15} position="left" angle={-90} style={{ textAnchor: 'middle', fontSize: '80%', fill: 'rgba(255, 255, 255, 0.87)' }} />
              </YAxis>
