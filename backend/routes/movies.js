        ...
        .then(function (response) {
            if (response.data) {
                ...
                if (response.data.Actors)
                    ...
                    obj.actors = "N/A";
                if (response.data.Language)         ///nouveau language
                    obj.language = response.data.Language;
                else
                    obj.language = "N/A";
                if (response.data.Type)         ///nouveau type
                    obj.type = response.data.Type;
                else
                    obj.type = "N/A";
                if (response.data.Genre)         ///nouvelle catégories genre
                    obj.genre = response.data.Genre;
                else
                    obj.genre = "N/A";
                ...
                if (response.data.BoxOffice)
                    ...
                    if(obj.boxOffice=="N/A"){
                        var min=1; 
                        var max=500;  
                        var random = Math.floor(Math.random() * (max - min)) + min;  
                        obj.boxOffice = random;
                    }
                else{
                    obj.boxOffice = "N/A";
                }
